...
router.patch('/:id/reschedule', async (req, res) => {
  try {
    ...
    const available = await isSlotAvailable(appointment.serviceType, newDate, timeSlot, appointment._id);
    if (!available) {
      return res.status(409).json({ error: 'Requested new slot is not available' });
    }

    appointment.date = newDate;
    appointment.timeSlot = timeSlot;
    appointment.status = 'Rescheduled';
    appointment.history.push({ status: 'Rescheduled', date: newDate, timeSlot });
    await appointment.save();
